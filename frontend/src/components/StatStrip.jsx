...
const KPI_DEFS = [
  ...
  { key: 'focused', label: '已关注 / 已投标', sub: '已介入跟进' },
  { key: 'closed', label: '已中标 / 已结束', sub: '终态记录' },
  { key: 'last_run_at', label: '最近抓取', sub: 'cron / 手动', isDate: true },
];
...
export default function StatStrip({ stats }) {
  ...
  const focused = (byRev['A.关注中'] || 0) + (byRev['H.已投标'] || 0);
  const closed = (byRev['Y.未中标'] || 0) + (byRev['Z.已中标'] || 0);
  ...
  const values = {
    ...
    focused,
    closed,
    last_run_at: stats.last_run_at ? fmt.dateTime(stats.last_run_at) : '—',
  };
