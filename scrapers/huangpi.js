 ...
 *
 * 注意：列表只返回 uuid/titleName/fbTime，详情需合并 list 记录。
...
const TENANT_CODE = '704156826496442368';
const HEADERS = {
...
// 合并 list 记录进 detail（黄陂区列表只返 3 字段）
...
module.exports = createPlatform({
  ...
  fields: {
    ...
    detailUrl: r => `http://47.111.115.168:10013/#/infoDetail?id=${r.uuid}&type=remote`,
  ...
  inferScopeRules: HUANGPI_SCOPE_RULES,
  detailDelayMs: 300
});

if (require.main === module) {
  module.exports.run({ pageCount: 1, pageSize: 5, outputFile: 'huangpi_test.json' })
    .then(() => process.exit(0))
    .catch(e => {
      console.error('失败:', e);
      process.exit(1);
